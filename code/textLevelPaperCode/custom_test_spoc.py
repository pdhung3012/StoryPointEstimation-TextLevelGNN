...
from custom_data_helper_spoc import DataHelper
import numpy as np
import tqdm
import sys, random
import argparse
...
import os
from custom_pmi_spoc import cal_PMI
...
fopDataset='../../../../dataPapers/textInSPOC/mixCode_v2/step6/'
fnSystem='tlGNN'
fpLabel=fopDataset+fnSystem+"/test_label.txt"
fpPred=fopDataset+fnSystem+"/test_pred.txt"
fpResultSEEShort=fopDataset+"/resultSPOC_"+fnSystem+"_short.txt"
fpResultSEEDetails=fopDataset+"/resultSPOC_"+fnSystem+"_details.txt"
...
def test(model_name, dataset,dictLabel):
    model = torch.load(os.path.join('.',  'model_tlGNN.pkl'))
    ...
    for content, label, _ in data_helper.batch_iter(batch_size=64, num_epoch=1):
        ...
        for i in range(0,len(label)):
            list_pred.append((dictLabel[int(pred[i])]))
            list_label.append((dictLabel[int(label[i])]))
